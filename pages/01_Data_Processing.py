    ...
    if admin_file is None or dist_file is None:
        ...
        try:
            ...
            def find_and_rename_cols(df, col_type):
                ...
                """
                # Define a dictionary of potential column name fragments for each key type
                patterns = {
                ...
                # Get the relevant patterns for the current file type
                if col_type == 'admin':
                ...
                # Search for each final column name
                for final_name, prefixes in relevant_patterns.items():
                    for raw_col in df.columns:
                        for prefix in prefixes:
                            if raw_col.startswith(prefix):
                                if final_name not in found_cols: # Only rename the first match
